...
fn roll_normal<T: Stat>(stat: &T, raw_stat_value: f32, modifiers: CheckModifiers) -> u16 {
...
pub struct VsParticipant<T: Stat> {
...
impl Stats {
    ...
    pub fn check<T: Stat>(
        ...
        if let Some(stats) = world.get::<Stats>(entity) {
            check(&stat, stat.get_value(stats, world), modifiers, difficulty)
        } else {
